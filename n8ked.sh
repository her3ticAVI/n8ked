...
# Prints every leading '#'-comment line after the shebang, stopping at the
# first non-comment line — so the help text always matches the actual
# header block instead of a hardcoded line range that could silently
# truncate or over-print if the header ever grows or shrinks.
print_help() { awk 'NR==1{next} /^#/{print; next} {exit}' "$0" | sed 's/^# \{0,1\}//'; }
while [[ $# -gt 0 ]]; do
    case "$1" in
...
# An array (not a flags string) so each flag is its own element — no
# reliance on word-splitting an unquoted variable, so a future flag that
# contains a space or glob character can't silently misbehave.
CURL_ARGS=(curl -skL --post301 --post302 --post303 --max-time 10)
TMPDIR=$(mktemp -d)
...
import re, sys
PATTERNS = [
    ("AWS Access Key ID",        r"AKIA[0-9A-Z]{16}", 0),
    ("AWS Secret Access Key",    r"(?i)aws(?:.{0,20})?secret(?:.{0,20})?[\"\x27]?\s*[:=]\s*[\"\x27]([0-9a-zA-Z/+]{40})[\"\x27]", 1),
    ("Generic API key",          r"(?i)(?:api[_-]?key|apikey)[\"\x27]?\s*[:=]\s*[\"\x27]([A-Za-z0-9_\-]{16,})[\"\x27]", 1),
    ("Bearer token",             r"Bearer [A-Za-z0-9\-_\.]{20,}", 0),
    ("JWT",                      r"eyJ[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+\.[A-Za-z0-9_-]{5,}", 0),
    ("Slack token",              r"xox[baprs]-[0-9A-Za-z-]{10,}", 0),
    ("Private key block",        r"-----BEGIN (?:RSA |EC |OPENSSH |DSA )?PRIVATE KEY-----", 0),
    ("Generic password field",   r"(?i)\"password\"\s*:\s*\"([^\"\s]{3,})\"", 1),
    ("Generic secret field",     r"(?i)\"(?:secret|token|client_secret|access_token|refresh_token)\"\s*:\s*\"([^\"\s]{3,})\"", 1),
    ("DB connection string",     r"(?i)((?:mongodb|postgres(?:ql)?|mysql|redis)://[^\s\"\x27]{6,})", 1),
...
def main():
    ...
    try:
        ...
        return
    seen = set()
    for name, pat, group_idx in PATTERNS:
        for m in re.finditer(pat, body):
            val = m.group(group_idx) if group_idx else m.group(0)
            if val is None:
                continue
...
n8n_try_login() {
    ...
    local resp
    resp=$("${CURL_ARGS[@]}" -X POST "$tgt/rest/login" -H "Content-Type: application/json" \
...
webhook_probe() {
    ...
    local status
    status=$(timeout 9 "${CURL_ARGS[@]}" -o "$bf" -w '%{http_code}' -X "$method" "$tgt/$base/$cand" 2>/dev/null)
    local curl_exit=$?
...
ver_suffix() {
    # Whatever trails the numeric x.y.z(.w) prefix, e.g. "-beta.1", "-rc2".
    # A non-empty suffix marks a pre-release build.
    local v="$1" norm
    norm=$(ver_norm "$v")
    echo "${v#"$norm"}"
}
ver_cmp() {
    ...
    # Numeric prefixes are equal — a pre-release suffix (-beta, -rc, ...)
    # sorts BEFORE the bare release it precedes (1.65.0-rc1 < 1.65.0), so a
    # pre-release build is correctly still treated as "not yet the fixed
    # version" by ver_ge/ver_lt range checks.
    local sa sb
    sa=$(ver_suffix "$1"); sb=$(ver_suffix "$2")
    if [[ -z "$sa" && -z "$sb" ]]; then
        echo 0
    elif [[ -z "$sa" ]]; then
        echo 1
...
scan_target() {
    ...
    ROOT_HEADERS=$("${CURL_ARGS[@]}" -D - -o "$ROOT_BODY_FILE" "$TARGET/" 2>/dev/null)
    ...
    SETTINGS_JSON=$("${CURL_ARGS[@]}" "$TARGET/rest/settings" 2>/dev/null)
    HEALTHZ_JSON=$("${CURL_ARGS[@]}" "$TARGET/healthz" 2>/dev/null)
    local WEBHOOK_BODY_FILE="$TMPDIR/webhook.body"
    WEBHOOK_STATUS=$("${CURL_ARGS[@]}" -o "$WEBHOOK_BODY_FILE" -w '%{http_code}' "$TARGET/webhook-test/" 2>/dev/null)
    ...
    WEBHOOK_PROD_STATUS=$("${CURL_ARGS[@]}" -o "$WEBHOOK_PROD_BODY_FILE" -w '%{http_code}' "$TARGET/webhook/" 2>/dev/null)
...
' 2>/dev/null
    ...
    for entry in "${ENDPOINT_RESULTS[@]}"; do
        ...
        local head_status
        head_status=$("${CURL_ARGS[@]}" -o /dev/null -w '%{http_code}' -I "$TARGET$ep" 2>/dev/null)
    ...
    CORS_HEADERS=$("${CURL_ARGS[@]}" -D - -o /dev/null -H "Origin: https://n8ked-cors-probe.invalid" "$TARGET/rest/settings" 2>/dev/null)
    ...
    for p in "${EXPOSE_PATHS[@]}"; do
        ...
        local st
        st=$("${CURL_ARGS[@]}" -o "$bf" -w '%{http_code}' "$TARGET$p" 2>/dev/null)
    ...
    if [[ "$CHECK_LOCKOUT" == true ]]; then
        ...
        for li in 1 2 3 4 5 6 7 8; do
            local lstatus
            lstatus=$("${CURL_ARGS[@]}" -o /dev/null -w '%{http_code}' -X POST "$TARGET/rest/login" \
                -H "Content-Type: application/json" \
    ...
    local -a CVE_POC_ROWS=()
    local -a CVE_POC_SOURCES=()
